...
if __name__ == "__main__":
    ...
    dataset, iteration, pipe = model.extract(
        args), args.iteration, pipeline.extract(args)
    ...
    if not args.skip_mesh:
        ...
        if args.unbounded:
            ...
            name = 'fuse.ply'
            depth_trunc = (gaussExtractor.radius *
                           2.0) if args.depth_trunc < 0 else args.depth_trunc
            voxel_size = (
                depth_trunc /
                args.mesh_res) if args.voxel_size < 0 else args.voxel_size
            sdf_trunc = 5.0 * voxel_size if args.sdf_trunc < 0 else args.sdf_trunc
            mesh = gaussExtractor.extract_mesh_bounded(voxel_size=voxel_size,
                                                       sdf_trunc=sdf_trunc,
        ...
        o3d.io.write_triangle_mesh(
            ...
            mesh_post)
        print("mesh post processed saved at {}".format(
            os.path.join(train_dir, name.replace('.ply', '_post.ply'))))
