...
def readCamerasFromTransforms(path,
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            bg = np.array([1, 1, 1]) if white_background else np.array(
                [0, 0, 0])

            norm_data = im_data / 255.0
            arr = norm_data[:, :, :3] * norm_data[:, :, 3:4] + bg * (
                1 - norm_data[:, :, 3:4])
            image = Image.fromarray(np.array(arr * 255.0, dtype=np.byte),
                                    "RGB")
...
def readPolycamSceneInfo(path, llffhold=8):

    with open(os.path.join(path, 'transforms.json')) as file:
        meta = json.load(file)
    poses = []
    ...
    scene_info = load_camera(meta, poses, path, llffhold)

    return scene_info


def load_camera(meta, poses, path, llffhold):
    cam_infos = []
    ...
    for num_id, frame in enumerate(meta["frames"]):
        ...
        trans[2, :] *= -1
        trans = trans[np.array([0, 2, 1, 3]), :]

        # Convert from nerfstudio(OpenGL) to ours (OpenCV) COLMAP's camera coordinate system
        trans[0:3, 1:3] *= -1  # camera to world
        # world to camea
        trans = np.linalg.inv(trans)

        im = Path(frame["file_path"])
        ...
        if "fl_x" in meta.keys():
            FovY = focal2fov(meta["fl_x"], height)
            FovX = focal2fov(meta["fl_x"], width)
        else:
            FovY = focal2fov(frame["fl_x"], height)
            FovX = focal2fov(frame["fl_x"], width)

        cam_infos.append(
            CameraInfo(
                uid=num_id,
                R=np.transpose(trans[:3, :3]),
                T=np.array(trans[:3, 3]),
                FovY=FovY,
                FovX=FovX,
                image=image,
                image_path=image_path,
    ...
    off = np.mean(poses[:, :3, -1].numpy(), axis=0)
    max_pos = np.max(poses[:, :3, -1].numpy(), axis=0)
    min_pos = np.min(poses[:, :3, -1].numpy(), axis=0)
    if eval:
        train_cam_infos = [
            c for idx, c in enumerate(cam_infos) if idx % llffhold != 0
        ]
