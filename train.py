...
def training(dataset, opt, pipe, testing_iterations, saving_iterations,
             checkpoint_iterations, checkpoint, camera_type):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians, camera_type)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            ema_normal_for_log = 0.4 * normal_loss.item(
            ) + 0.6 * ema_normal_for_log
            ...
            if tb_writer is not None:
                tb_writer.add_scalar('train_loss_patches/dist_loss',
                                     ema_dist_for_log, iteration)
            ...
            while network_gui.conn != None:
                try:
                    ...
                    if custom_cam != None:
                        ...
                        net_image = render_net_image(render_pkg,
                                                     dataset.render_items,
                                                     render_mode, custom_cam)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed,
    ...
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/reg_loss', Ll1.item(),
                             iteration)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    gt_image = torch.clamp(viewpoint.original_image.to("cuda"),
                                           0.0, 1.0)
                    if tb_writer and (idx < 5):
                        ...
                        try:
                            ...
                            tb_writer.add_images(config['name'] +
                                                 "_view_{}/rend_normal".format(
                                                     viewpoint.image_name),
                                                 rend_normal[None],
                                                 global_step=iteration)
                            tb_writer.add_images(config['name'] +
                                                 "_view_{}/surf_normal".format(
                                                     viewpoint.image_name),
                                                 surf_normal[None],
                                                 global_step=iteration)
                            tb_writer.add_images(config['name'] +
                                                 "_view_{}/rend_alpha".format(
                                                     viewpoint.image_name),
                                                 rend_alpha[None],
                                                 global_step=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--camera_type", type=str, default="polycam")
